...
FlxG.cameras.add(camCinematics = new HudCamera(), false);
...
function create() {
    for (i in 0...2) {
        ...
        cinematicBar.cameras = [camCinematics];
...
function onEvent(eventEvent) {
    ...
    if (eventEvent.event.name == "Cineamatic Bars") {
        if (params[0] == false)
            ...
            for (bar in [cinematicBar1, cinematicBar2]) {
                var tween:FlxTween = FlxTween.tween(bar.scale, {y: (FlxG.height/2) * params[1]}, params[2], {ease: Reflect.field(FlxEase, flxease)});
