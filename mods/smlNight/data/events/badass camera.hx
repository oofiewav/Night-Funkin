...
{
	if (__event.event.name == "badass camera")
	{
		...
		var time:Float = __event.event.params[3];
		...
		FlxTween.tween(camGame, {angle: rotate}, time, {ease: easeDirection});
		FlxTween.tween(camGame, {zoom: zoom}, time, {ease: easeDirection, onUpdate: function(value) {defaultCamZoom = FlxG.camera.zoom;}});
	}
}
